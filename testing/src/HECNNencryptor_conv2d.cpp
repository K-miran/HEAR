 ...
 */
void HECNNenc::encryptdata(vector<Ciphertext> &res, vector<vector<vector<double>>> data, double scale)
{
    res.resize(2);
 ...
 */
void HECNNenc::encryptdata_packed(Ciphertext &res, vector<vector<vector<double>>> data, double scale)
{
...
*/
void HECNNenc::decrypt_vector(vector<double> &dmsg, Ciphertext ct){
    Plaintext pmsg;
...
/*
 @param[in] kernel, the kernel of the first convoluational layer
                    [out_channels][in_channels][filter_height][filter_width]; [64][2][3][3] when nch=64 and [128][2][3][3] when nch=128
 ...
 @param[out] ker_poly, the encoded polynomials of kernels as a native plaintext format (size = [NB_CHANNELS[1]/16][2][9])
 The ouput polynomials are scaled by the factor of "scale" at "level"
 */
void HECNNenc::encode_conv1(vector<vector<vector<Plaintext>>> &ker_poly, ften kernel, vector<int> NB_CHANNELS, double scale, int level)
{
    int ncols = NB_CHANNELS[1] / 16;
    int nrows = NB_CHANNELS[0];
    
    ker_poly.resize(ncols, vector<vector<Plaintext>>(nrows, vector<Plaintext> (9)));    // [8][2][9]
    size_t slot_count = encoder.slot_count();
    int num = ncols * Param_conv2d::DIM2_FILTERS;
    
    MT_EXEC_RANGE(num, first, last);
    for(int n = first; n < last; ++n){
        int j = (int) floor((double) n / (double) Param_conv2d::DIM2_FILTERS);  // 0 <= j < ncols
        int j_start = (j << 4);
        int option = (n % Param_conv2d::DIM2_FILTERS); // 0 <= option < 9
        ...
        for(int i = 0; i < nrows; ++i){
            for(int k = 0; k < 16; ++k){
                ...
                ker1_to_vector(temp_short, kernel[j_start + k][i], option);
...
/*
 ...
 The ouput polynomials are scaled by the factor of "scale" at "level"
*/
void HECNNenc::encode_conv1(vector<vector<vector<Plaintext>>> &ker_poly, ften kernel, string mode,
                            vector<int> NB_CHANNELS, double scale, int level)
{
    int ncols = NB_CHANNELS[1] / 16;
    int nrows = NB_CHANNELS[0];
    
    ker_poly.resize(ncols, vector<vector<Plaintext>>(nrows, vector<Plaintext> (9)));
    size_t slot_count = encoder.slot_count();
    ...
    if(mode == "fully"){
        ...
        for(int n = first; n < last; ++n){
            ...
            for(int k = 0; k < 16; ++k){
                dvec temp_short;    // size = (32 * 16)
                bool i1 = (k % 2);
                ker1_to_vector(temp_short, kernel[j_start + k][i1], option);
        ...
        for(int n = first; n < last; ++n){
            ...
            msgrightRotate_inplace(temp_full_slots[1], Param_conv2d::shift);
            encoder.encode(temp_full_slots[1], scale, level, ker_poly[j][1][option]);
        }
        MT_EXEC_RANGE_END
    }
...
/* -----------------------------------------------
 * Encoding weights of the second/third layer blocks
 * -----------------------------------------------
 */
...
/*
 ...
 The ouput polynomials are scaled by the factor of "scale" at "level"
*/
void HECNNenc::encode_conv(vector<vector<vector<Plaintext>>> &ker_poly, ften kernel, int conv_block, string mode,
                           vector<int> NB_CHANNELS, double scale, int level)
{
    size_t slot_count = encoder.slot_count();
    int conv_block1 = conv_block - 1;
    size_t num_out = (NB_CHANNELS[conv_block] / 16);
    size_t num_in = NB_CHANNELS[conv_block1];
    int dist = (conv_block == 2 ? 2 : 4);
    
    ker_poly.resize(num_out, vector<vector<Plaintext>>(num_in, vector<Plaintext> (9)));
    
    if(mode == "fully"){
        for(size_t i = 0; i < num_out; ++i){ // poly[i][j][-]
            MT_EXEC_RANGE(num_in, first, last);
            for(int j = first; j < last; ++j){
                for(int option = 0; option < Param_conv2d::DIM2_FILTERS; ++option){
                    ...
                    for(int j1 = j; j1 < jend; ++j1){
                        dvec temp;
                        ker2_to_vector(temp, kernel[i * 16 + (j1 - j)][j1], option, dist);
                        temp_slots.insert(temp_slots.end(), temp.begin(), temp.end());
                    }
                    for(int j1 = jstart; j1 < j; ++j1){
                        dvec temp;
                        ker2_to_vector(temp, kernel[i * 16 + (16 - j + j1)][j1], option, dist);
                        temp_slots.insert(temp_slots.end(), temp.begin(), temp.end());
                    }
        ...
        for(size_t i = 0; i < num_out; ++i){
            ...
            for(int j = first; j < last; ++j){
                for(int option = 0; option < Param_conv2d::DIM2_FILTERS; ++option){
                    ...
                    if((j % 16) != 0) {
                        msgrightRotate_inplace(temp_slots, (j % 16) * Param_conv2d::shift);
                    }
                    encoder.encode(temp_slots, scale, level, ker_poly[i][j][option]);
                }
            }
        ...
        for(size_t i = 0; i < num_out; ++i){
            ...
            for(int j = first; j < last; ++j){
                for(int option = 0; option < Param_conv2d::DIM2_FILTERS; ++option){
                    ...
                    // e.g., k0 * rho(ct[1], -33) = rho((rho(k0, 33) * ct[1]), -33); k5 * rho(ct[5], 1) = rho((rho(k5, -1) * ct[5]), 1)
                    if(option < 4){
                        msgleftRotate_inplace(temp_slots, Param_conv2d::steps_conv[conv_block1][7 - option]);
                    } else if(option > 4){
                        msgrightRotate_inplace(temp_slots, Param_conv2d::steps_conv[conv_block1][option - 1]);
                    }
                    encoder.encode(temp_slots, scale, level, ker_poly[i][j][option]);
                }
            }
...
/*
 ...
 The ouput polynomials are scaled by the factor of "scale" at "level"
 */
void HECNNenc::encode_conv_interlaced(vector<vector<vector<Plaintext>>> &ker_poly,
                                      ften kernel, int conv_block, string mode, vector<int> NB_CHANNELS, double scale, int level)
{
    size_t slot_count = encoder.slot_count();
    int conv_block1 = conv_block - 1;
    size_t num_out = (NB_CHANNELS[conv_block] / 16);
    size_t num_in = NB_CHANNELS[conv_block1];
    int dist = (conv_block == 2 ? 2 : 4);
    int batch_in = (conv_block == 2 ? 4 : 16);  // the number of interlacing entries
    
    int actual_bath_in = num_in/16;
    if(actual_bath_in < batch_in){
        batch_in = actual_bath_in;
    }

    int num_in1 = (int) num_in/ batch_in;      // the number of actual outputs; e.g., (128/4) = 32, (256/16)= 16
    
    ker_poly.resize(num_out, vector<vector<Plaintext>>(num_in1, vector<Plaintext> (9)));    // [16][128][9]
    if(mode == "fully"){
            ...
            for(int option = 0; option < Param_conv2d::DIM2_FILTERS; ++option){
                for(int j = 0; j < num_in1; ++j){
                    ...
                    for(int j1 = j; j1 < jend; ++j1){
                        ...
                        ker2_to_interlaced_vector(temp, kernels, option, dist); // len = (32 * 16)
                        temp_slots.insert(temp_slots.end(), temp.begin(), temp.end());
                    }
                    ...
                    for(int j1 = jstart; j1 < j; ++j1){
                        ...
                        ker2_to_interlaced_vector(temp, kernels, option, dist);
                        temp_slots.insert(temp_slots.end(), temp.begin(), temp.end());
                    }
                    encoder.encode(temp_slots, scale, level, ker_poly[i][j][option]);
                }
...
*/
void HECNNenc::encode_act1(vector<vector<Plaintext>> &act_poly, dmat real_poly, int NB_CHANNELS,
                           double qscale, double qcscale, int level)
{
    ...
    int const_lvl = level - 3; // = Param_conv2d::ker_poly_lvl[0]
    ...
    for(int k = first; k < last; ++k){
        ...
        for(int j = 0; j < 16; ++j){
            dvec temp_short;    // size = (32 * 16)
            val1_to_vector(temp_short, real_poly[i1 + j][l], nrows, ncols);
 ...
 */
void HECNNenc::encode_act(vector<vector<Plaintext>> &act_poly, dmat real_poly, int conv_block, int NB_CHANNELS,
                          double qscale, double qcscale, int level)
{
    size_t slot_count = encoder.slot_count();
    int nrows = 32;
    ...
    int num = (NB_CHANNELS / 16);                    // 2*nch/16 for B2; 4*nch for B3
    act_poly.resize(num, vector<Plaintext>(3));     // [2*nch/16][3] for B2; [4*nch/16][3] for B3
    
    int const_lvl = level - 3;                      // = Param_conv2d::ker_poly_lvl[conv_block - 1]
    int nonconst_lvl = level - 2;
    ...
    for(int k = first; k < last; ++k){
        ...
        for(int j = 0; j < 16; ++j){
            dvec temp_short;    // size = (32 * 16)
            val_to_vector(temp_short, real_poly[i1 + j][l], nrows, ncols, dist);
            temp_full_slots.insert(temp_full_slots.end(), temp_short.begin(), temp_short.end());
        }
...
void HECNNenc::encode_dense_kerpoly(vector<vector<Plaintext>> &dense_ker_poly, dmat dense_ker,
                                    int NB_CHANNELS, double scale, int level)
{
    size_t slot_count = encoder.slot_count();
    size_t numi = NB_CHANNELS/16;
    ...
    for(int i = first; i < last; ++i){
        ...
        for(int j = 0; j < 7; ++j){
            ...
            for(int l = 0; l < 10; ++l){
                int l1 = ((j + l) << Param_conv2d::logshift);
        ...
        for(int j = 7; j < 16; ++j){
            ...
            for(int l = 0; l < j - 6; ++l){
                int l1 = (l << Param_conv2d::logshift);
                int xcord = (i1 + l);
 ...
 */
void HECNNenc::encode_dense_biaspoly(Plaintext &bias_poly, dvec dense_bias, double scale)
{
    size_t slot_count = encoder.slot_count();
    int lvl = 0;
    ...
    for(int i = 0; i < (int) dense_bias.size(); ++i){
        int i1 = Param_conv2d::shift * i; // 512, 512*2, ...
...
*/
void HECNNenc::prepare_network(vector<vector<vector<Plaintext>>>&ker1, vector<vector<Plaintext>>&act1,
...
*/
void HECNNenc::prepare_network_interlaced(vector<vector<vector<Plaintext>>>&ker1, vector<vector<Plaintext>>&act1,
                                         vector<vector<vector<Plaintext>>>&ker2, vector<vector<Plaintext>>&act2,
                                         vector<vector<vector<Plaintext>>>&ker3, vector<vector<Plaintext>>&act3,
...
*/
void HECNNenc::prepare_network_naive(vector<vector<vector<Plaintext>>>&ker1, vector<vector<Plaintext>>&act1,
                    vector<vector<vector<Plaintext>>>&ker2, vector<vector<Plaintext>>&act2,
                    vector<vector<vector<Plaintext>>>&ker3, vector<vector<Plaintext>>&act3,
                    vector<vector<Plaintext>> &dense_ker_poly, Plaintext &dense_bias_poly,
                    vector<ften> ker, vector<dmat> real_poly, dmat dense_ker, dvec dense_bias,
...
/*
 ...
 In particular, all the parameters are encoded at the highest encoding level (without the level-aware encoding method).
*/
void HECNNenc::prepare_network_interlaced_naive(vector<vector<vector<Plaintext>>>&ker1, vector<vector<Plaintext>>&act1,
                                         vector<vector<vector<Plaintext>>>&ker2, vector<vector<Plaintext>>&act2,
                                         vector<vector<vector<Plaintext>>>&ker3, vector<vector<Plaintext>>&act3,
                                         vector<vector<Plaintext>> &dense_ker_poly, Plaintext &dense_bias_poly, vector<Plaintext> &zero_one_poly,
                                         vector<ften> ker, vector<dmat> real_poly, dmat dense_ker, dvec dense_bias,
