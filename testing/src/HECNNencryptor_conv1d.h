...
public:
    ...
    HECNNenc1d(Encryptor& encryptor, Decryptor& decryptor, CKKSEncoder& encoder):
            encryptor(encryptor), decryptor(decryptor), encoder(encoder) {}
    ...
    void encode_conv1(vector<vector<vector<Plaintext>>> &ker_poly, dten kernel, vector<int> NB_CHANNELS, double scale, int level);
    void encode_conv1(vector<vector<vector<Plaintext>>> &ker_poly, dten kernel, string mode, vector<int> NB_CHANNELS, double scale, int level);
    
    // Encoding weights for the second/their convolutional layer
    void encode_conv(vector<vector<vector<Plaintext>>> &ker_poly, dten kernel, int conv_block, string mode, vector<int> NB_CHANNELS, double scale, int level);
    void encode_conv_interlaced(vector<vector<vector<Plaintext>>> &ker_poly, dten kernel,
                                int conv_block, string mode, vector<int> NB_CHANNELS, double scale, int level);

    // Encoding of activation
    void encode_act1(vector<vector<Plaintext>> &act_poly, dmat real_poly, int NB_CHANNELS, double qscale, double qcscale, int level);
    void encode_act(vector<vector<Plaintext>> &act_poly, dmat real_poly, int conv_block, int NB_CHANNELS, double qscale, double qcscale, int level);
