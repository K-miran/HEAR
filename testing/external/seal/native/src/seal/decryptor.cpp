...
namespace seal
{
    ...
    // miran
    void Decryptor::decrypt_ckks(const Ciphertext &encrypted, Plaintext &destination)
    {
        // Verify that encrypted is valid.
        if (!is_valid_for(encrypted, context_))
        {
        ...
        {
            case scheme_type::BFV:
                ckks_decrypt_naive(encrypted, destination, pool_);
                return;
    ...
    void Decryptor::ckks_decrypt_naive(const Ciphertext &encrypted,
