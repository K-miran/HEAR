...
namespace seal
{
    namespace util
    {
        ...
        // miran: used for evaluator.add_const_inplace
        inline void add_poly_const_coeffmod(const std::uint64_t *operand1,
            const std::uint64_t plain, std::size_t coeff_count,
            const SmallModulus &modulus, std::uint64_t *result)
        {
    #ifdef SEAL_DEBUG
            if (operand1 == nullptr && coeff_count > 0)
            ...
            for (; coeff_count--; result++, operand1++)
            {
                std::uint64_t sum = *operand1 + plain;
        ...
        // miran
        void apply_galois_ntt_fast(const std::uint64_t *input,
                                   std::uint64_t *gal_ntt_index, int coeff_count, std::uint64_t *result);
        ...
        // miran
        void dyadic_product_coeffmod_fast(const std::uint64_t *operand1,
                const std::uint64_t *operand2, std::size_t coeff_count,
                const SmallModulus &modulus, std::uint64_t *result);
