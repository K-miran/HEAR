...
namespace seal
{
    namespace util
    {
        ...
        // miran
        void apply_galois_ntt_fast(const uint64_t *input, uint64_t *gal_ntt_index, int coeff_count, uint64_t *result)
        {
            for (size_t i = 0; i < coeff_count; i++)
            {
                //uint64_t index = gal_ntt_index[i];
                result[i] = input[gal_ntt_index[i]];
            }
        }
        ...
        // miran
        void dyadic_product_coeffmod_fast(const uint64_t *operand1,
            const uint64_t *operand2, size_t coeff_count,
            const SmallModulus &modulus, uint64_t *result)
        {
#ifdef SEAL_DEBUG
            ...
            for (; coeff_count--; operand1++, operand2++, result++)
            {
                unsigned long long z = *operand1 * *operand2;
                
                unsigned long long tmp;
                multiply_uint64_hw64(z, const_ratio_1, &tmp);               // tmp <- z * r mode 2^{64}
                tmp = z - tmp * modulus_value;                             // Barrett subtraction
                *result = tmp - (modulus_value & static_cast<uint64_t>(     // result <- tmp[0] (mod q)
                           -static_cast<int64_t>(tmp >= modulus_value)));
            }
        }
