    ...
    */
    ...
    public:
        ...
        */
        ...
        // miran
        void add_plain_inplace_ckks(Ciphertext &encrypted, const Plaintext &plain);
        ...
        /**
         ...
         @throws std::logic_error if result ciphertext is transparent
         */
        void add_plain_leveled_inplace(Ciphertext &encrypted, const Plaintext &plain);
        ...
        /**
         ...
         @throws std::logic_error if result ciphertext is transparent
         */
        inline void add_plain_leveled(const Ciphertext &encrypted, const Plaintext &plain,
                                      Ciphertext &destination)
        {
            destination = encrypted;
            add_plain_leveled_inplace(destination, plain);
        }
        
        // miran
        void add_const_inplace(Ciphertext &encrypted, const std::vector<std::uint64_t> &plain);
        inline void add_const(const Ciphertext &encrypted, const std::vector<std::uint64_t> &plain,
                              Ciphertext &destination)
        {
            destination = encrypted;
            add_const_inplace(destination, plain);
        }
        
        // miran
        void multiply_plain_inplace_ckks(Ciphertext &encrypted, const Plaintext &plain,
            MemoryPoolHandle pool = MemoryManager::GetPool());
        
        
        /*
         Multiplies a ciphertext with a plaintext (encoded at some specific level)
         */
        // miran
        void multiply_plain_leveled_inplace(Ciphertext &encrypted, const Plaintext &plain,
                                            MemoryPoolHandle pool = MemoryManager::GetPool());
        // miran
        inline void multiply_plain_leveled(const Ciphertext &encrypted,
                                           const Plaintext &plain, Ciphertext &destination,
                                           MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            destination = encrypted;
            multiply_plain_leveled_inplace(destination, plain, std::move(pool));
        }
        
        // miran, scalar multiplication when qi's < 2^31
        void multiply_plain_leveled_inplace_fast(Ciphertext &encrypted, const Plaintext &plain,
                                            MemoryPoolHandle pool = MemoryManager::GetPool());
        // miran
        inline void multiply_plain_leveled_fast(const Ciphertext &encrypted,
                                           const Plaintext &plain, Ciphertext &destination,
                                           MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            destination = encrypted;
            multiply_plain_leveled_inplace_fast(destination, plain, std::move(pool));
        }
        
        void multiply_const_inplace(Ciphertext& encrypted, const std::vector<std::uint64_t> &plain,
                                    MemoryPoolHandle pool = MemoryManager::GetPool());
        
        inline void multiply_const(const Ciphertext &encrypted,
                                const std::vector<std::uint64_t> &plain, Ciphertext &destination,
                                MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            destination = encrypted;
            multiply_const_inplace(destination, plain, std::move(pool));
        }
        ...
        /**
         ...
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        inline void rotate_rows_many(std::vector<Ciphertext> &encrypted,
                                     std::vector<int> steps, const GaloisKeys &galois_keys,
                                     MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            if (context_->key_context_data()->parms().scheme() != scheme_type::BFV)
            {
            ...
            rotate_internal_many(encrypted, steps, galois_keys, std::move(pool));
        }
        ...
        /**
         ...
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        inline void rotate_rows_many(const Ciphertext &encrypted, std::vector<int> steps,
                                     const GaloisKeys &galois_keys, std::vector<Ciphertext> &destination,
                                     MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            destination[0] = encrypted;
            rotate_rows_many(destination, steps, galois_keys, std::move(pool));
        }
        
        inline void rotate_vector_many(std::vector<Ciphertext> &encrypted,
                                       std::vector<int> steps, const GaloisKeys &galois_keys,
                                       MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            if (context_->key_context_data()->parms().scheme() != scheme_type::CKKS)
        ...
        inline void rotate_vector_many(const Ciphertext &encrypted, std::vector<int> steps,
                                       const GaloisKeys &galois_keys, std::vector<Ciphertext> &destination,
                                       MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            destination[0] = encrypted;
            rotate_vector_many(destination, steps, galois_keys, std::move(pool));
        }
        
        
        // miran
        /**
         Applies Galois many automorphisms to the same input ciphertext.
         The performance improvement comes from an precomputation of util::apply_galois_ntt
         ...
         @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
         */
        void apply_galois_many_fast(std::vector<Ciphertext> &encrypted,
                                    std::vector<uint64_t> galois_elt, const GaloisKeys &galois_keys,
                                    MemoryPoolHandle pool = MemoryManager::GetPool());
    ...
    private:
        ...
        // miran
        void rotate_internal_many(std::vector<Ciphertext> &encrypted, std::vector<int> steps,
                                  const GaloisKeys &galois_keys, MemoryPoolHandle pool);
        ...
        // miran
        void multiply_plain_normal_ckks(Ciphertext &encrypted, const Plaintext &plain,
                                   util::MemoryPool &pool);
        // miran
        void add_plain(
        ...
        // miran
        void multiply_plain_ntt_leveled(Ciphertext &encrypted_ntt, const Plaintext &plain_ntt);
        
        // miran
        void multiply_plain_ntt_leveled_fast(Ciphertext &encrypted_ntt, const Plaintext &plain_ntt);
