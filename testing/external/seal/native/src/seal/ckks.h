    ...
    */
    ...
    public:
        ...
        */
        ...
        // miran (encode when we are given the specific level of input message)
        template<typename T,
            typename = std::enable_if_t<std::is_same<T, double>::value ||
            std::is_same<T, std::complex<double>>::value>>
            inline void encode(const std::vector<T> &values,
                           parms_id_type parms_id, double scale, long level, Plaintext &destination,
                           MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            encode_internal(values, parms_id, scale, level, destination, std::move(pool));
        }
        ...
        template<typename T,
            ...
            std::is_same<T, std::complex<double>>::value>>
            inline void encode(const std::vector<T> &values,
                           double scale, long level, Plaintext &destination,
                           MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            encode(values, context_->first_parms_id(), scale, level,
                   destination, std::move(pool));
        }
    ...
    private:
                             ...
                             MemoryPoolHandle pool)
        {
            ...
            auto &parms = context_data.parms();
            auto &coeff_modulus = parms.coeff_modulus();
            std::size_t coeff_mod_count = level + 1;    // total number of special primes (l=2: {q0,q1,q2})
            std::size_t coeff_count = parms.poly_modulus_degree();
            ...
            // update the parms.id of the destination (encoded plaintext)
            while((context_->get_context_data(destination.parms_id())->chain_index() > level)){
                auto context_data_ptr = context_->get_context_data(destination.parms_id());
                auto &context_data = *context_data_ptr;
                auto &next_context_data = *context_data.next_context_data();
                destination.parms_id() = next_context_data.parms_id();
            }
        }
